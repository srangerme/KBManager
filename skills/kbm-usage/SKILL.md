...
- 每个 payload 必须包含 `dry_run: true | false`。
- `dry_run: true` 只做请求验证和前置条件检查；不写 object files，不移动文件，不执行 LLM resume。
- `dry_run: false` 执行真实操作，但仍必须满足 review gate 和 resume 规则。
...
- LLM output、generated drafts、candidate text、suggestions、dry-run result 都不是用户批准。
...
```json
{"entrypoint": "claude_code", "dry_run": false}
```
...
```json
{"entrypoint": "claude_code", "dry_run": false, "candidate_id": "knowledge-..."}
```
...
```json
{"entrypoint": "claude_code", "dry_run": false, "note_id": "note-..."}
```
