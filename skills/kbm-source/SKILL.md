---
name: kbm-source
description: 添加或废弃 KBManager source，包括本地 PDF、Markdown 单文件摄取。创建 knowledgebase 时提供的临时材料上下文不使用此 skill。
---
...
用于本地单文件 source 摄取。仅当用户意图是登记 KBManager source 时使用。
...
- 只有本地 `.md` 或 `.pdf` 单文件可以作为 `input_path` 交给 `kb.source.add`。
- 目录输入不属于 `kb.source.add` 支持范围。
- 不要直接把输入文件复制成 KBManager object；source object 由 API 写入。
...
- Knowledgebase create 中的 source-like context 不走 source add，不创建 candidate，不写持久 source；遇到这种意图时改用 kbm-kb 而不是 source lifecycle。
- Source summary 和 tags 由 `needs_llm` flow 生成，但不能覆盖 source fact fields。
